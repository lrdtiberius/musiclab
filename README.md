# MusicLab v2.0.0 Synology Project Safe
...
- Button-Platzierung aus v2.0.0 bleibt erhalten.
...
- sichtbare Versionsanzeige im Header korrigiert
- inkrementeller Schnellscan: unveränderte Dateien werden nicht erneut mit Mutagen geöffnet
- Medienseite lädt Grundlisten parallel und cached Interpreten-Alben
- Albumwechsel lädt nur noch die Titel statt die komplette Albumliste neu aufzubauen
- veraltete Medienanfragen können keine neuere Auswahl mehr überschreiben
- Header und Tabs an kleinere Browserfenster angepasst
- Protokollfilter haben feste Breiten und springen nicht mehr
- Release enthält absichtlich keine persönliche `musiclab.sqlite`


## Änderungen in 2.0.0

- Header unter 1500 px in zwei feste Zeilen aufgeteilt
- sämtliche Navigationstabs inklusive Einstellungen bleiben sichtbar
- Fortschrittstext wird bei wenig Platz sauber gekürzt
- Protokollfilter berücksichtigen nun auch das CSS-Element `Filter:`
- `Alle`, `Fehler`, `Scan`, `Tags`, `Audio` und `Sortierung` besitzen exakt gleiche Maße
- aktive Protokollfilter verändern die Größe nicht mehr
- zufälliger Umbruch von `Sortierung` in eine eigene Zeile entfernt


## Änderungen in 2.0.0

- dynamische Box `Auswahl: ... markiert` entfernt
- Tags-Filter `Suchen nach` wieder vollständig sichtbar
- Tags-Seitenleiste und Suchtyp-Auswahl erhalten stabile Mindestbreiten
- Protokoll-Hauptbereiche `Historie` und `Live-Log` auf exakt gleiche Breite fixiert
- dauerhafte Scrollbalken-Reserve verhindert Größenänderungen beim Filterwechsel
- Filterknöpfe ändern aktiv/inaktiv weder Breite noch Höhe, Rahmen oder Schriftgewicht
- Release-ZIP enthält keinen `tests`-Ordner mehr
